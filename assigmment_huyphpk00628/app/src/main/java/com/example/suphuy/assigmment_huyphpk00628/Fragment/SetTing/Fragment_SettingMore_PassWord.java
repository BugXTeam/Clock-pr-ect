...
this.fragmentManager=fm;
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        if(database.GetTKDNCount()>0)
        {
            try{
                if(database.getTKDN(1).getCode().equals("")){
                    tbtn_use_passcode.setChecked(false);
                }else
                    tbtn_use_passcode.setChecked(true);
            }catch (Exception e){{
    ...
    public void onClick(View view) {
            if(view==tbtn_use_passcode){
                if(tbtn_use_passcode.isChecked()){
                    ...
                    fragmentSettingInRight.setVisibility(View.VISIBLE);
                    fragmentManager.beginTransaction().replace(fragmentSettingInRight.getId(),
                            new Fragment_SettingMore_PassCode_Add(fragmentSettingInRight,false)).commit();
                    this.view.startAnimation(animationthoat);
                    fragmentSettingInRight.startAnimation(animation);
                }
